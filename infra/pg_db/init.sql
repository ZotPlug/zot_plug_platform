...
CREATE TABLE IF NOT EXISTS user_device_map (
  ...
  role VARCHAR(16) DEFAULT 'owner'                                  -- Device-level role
...
-- More fine-grained MQTT topic control:
CREATE TABLE device_topics (
  ...
  access_type VARCHAR(16) 
    CHECK (access_type IN ('publish', 'subscribe')),                -- Declares type of access
  PRIMARY KEY (device_id, topic, access_type)         
);