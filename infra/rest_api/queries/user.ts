...
export async function getAllUsers() {
    const result = await pool.query(
        'SELECT id, firstname, lastname, username, email FROM users WHERE is_deleted = FALSE'
    );
...
export async function getUserById(userId: number){
    const result = await pool.query(
        'SELECT * FROM users WHERE id = $1 AND is_deleted = FALSE', [userId]
    );
...
export async function addUser(firstname: string, lastname: string, username: string, email: string) {
    const result = await pool.query(
        'INSERT INTO users (firstname, lastname, username, email) VALUES ($1, $2, $3, $4) RETURNING id',
